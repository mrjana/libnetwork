...
func (nDB *NetworkDB) gossip() {
	...
	for nid, nodes := range networkNodes {
		kNodes := nDB.kRandomNodes(3, nodes)
		...
		if broadcastQ == nil {
			logrus.Errorf("Invalid broadcastQ encounted while gossiping for network %s", nid)
			continue
		}
...
func (nDB *NetworkDB) bulkSync(nid string, all bool) ([]string, error) {
	...
	if !all {
		...
		nodes = nDB.kRandomNodes(1, nodes)
	}

	log.Printf("%s: Initiating bulk sync with nodes %v", nDB.config.NodeName, nodes)
	var err error
	var networks []string
...
func (nDB *NetworkDB) bulkSyncNode(networks []string, node string, unsolicited bool) error {
	...
	log.Printf("%s: Initiating bulk sync for networks %v with node %s", nDB.config.NodeName, networks, node)
	...
	case <-ch:
		...
		log.Printf("%s: Bulk sync to node %s took %s", nDB.config.NodeName, node, time.Now().Sub(startTime))
	}
...
// kRandomNodes is used to select up to k random nodes. It is possible
// that less than k nodes are returned.
func (nDB *NetworkDB) kRandomNodes(k int, nodes []string) []string {
	n := len(nodes)
	kNodes := make([]string, 0, k)
OUTER:
