...
func (ovr *ovRouter) RegisterDriver(name string, driver driverapi.Driver, c driverapi.Capability) error {
	ovr.d = driver
	return nil
}
...
func main() {
	...
	ovr := &ovRouter{}
	if err := overlay.Init(ovr, opt); err != nil {
	...
	for {
		select {
		case <-sigCh:
			// ovr.d.Leave("testnetwork", "testep")
			overlay.Fini(ovr.d)
			os.Exit(0)
		}
	}
